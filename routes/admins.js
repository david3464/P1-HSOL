...
router.put('/:username/details/edit', async (req, res, next) => {
  ...
  try {
    ...
    if (admin == null) {
      console.log(
        'err during during put 3000/admins/:username/details/edit can not find admin on exist database' +
          err
      );
      res.redirect('/');
    } else {
      console.log(
        'err during 3000/admin/:username/details/edit update manager ' + err
      );
      res.redirect('/');
    }
  }
