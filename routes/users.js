...
//router address localhost:3000/users/register
//descriptions: Register User Handle
    ...
    User.findOne({username: username})
      .then(user =>{
        ...
        if(!user){
          ...
          // if(type === 'manager') {
          //   const newManager = new Manager ({username, first_name, last_name, address, post_code, city, email, mobile, branch});  
          //   User.saveManager(newUser, newManager, function(err, user){});
          //   // console.log(newUser);
          //   // console.log(newManager);
          //   req.flash('success_msg', 'You are now registered as Manager User and can login in')
          //   res.redirect('/users/login')
          // }
          // if(type === 'admin') {
          //   const newAdmin = new Admin ({username, first_name, last_name, address, post_code, city, email, mobile, });  
          //   User.saveManager(newUser, newAdmin, function(err, user){});
          //   // console.log(newUser);
          //   // console.log(newAdmin);
          //   req.flash('success_msg', 'You are now registered as Admin User and can login in')
          //   res.redirect('/users/login')
          // }
        }
      })
...
router.post('/register2', (req, res, next)=> {
  ...
  if (password.length < 6) {errors.push({ msg: 'Password must be at least 6 characters' });}
  if (errors.length > 0) {
    res.render('users/register_2', {errors,branch,role,type,username,first_name,last_name,address,post_code, city,email,mobile, });
  } else {
    //Validation Passed
    User.findOne({username: username})
      .then(user =>{
